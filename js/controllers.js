...
angular.module('webhooksio.controllers', [])
    .controller('WebhookCtrl', ['$scope', '$location', '$http', '$templateCache', 'consumerService',
        function($scope, $location, $http, $templateCache, consumerService) {
            ...
            if (!$scope.params.introduction_url) {
                $scope.introduction_url = 'partials/introduction-markdown.html';
            } else {
    ...
    .controller('DestinationCtrl', ['$scope', '$location', '$http', 'consumerService',
    ...
    .controller('DestinationAddCtrl', ['$scope', '$location', '$http', 'consumerService',
        function($scope, $location, $http, consumerService) {
            ...
            $scope.verify_ssl_options = [{
                option: 'true',
                name: 'Yes - ensure SSL cert is valid'
            }, {
                option: 'false',
                name: 'No - bypass validation checks on SSL cert'
            }];

            $scope.verify_ssl = $scope.verify_ssl_options[0].option;  //Default value
            ...
            consumerService.getAuthOptions($scope.urlbase, $scope.apiversion).success(function(data) {
                ...
                $scope.authentication_type = $scope.authoptions[0].type; // Default Value
            ...
            consumerService.getRetryPolicies($scope.urlbase, $scope.apiversion).success(function(data) {
                ...
                $scope.retry_policy_id = $scope.policies[0].policy_id; // Default Value
                    ...
                    for (var i = 0; i < $scope.policies.length; i++) {
                        if ($scope.policies[i].policy_id == newVal) {
                            if ($scope.policies[i].max_retry_count == null) {
                                ...
                                $('#retry_count').parsley('addConstraint', {
                                    required: true,
                                    type: 'number',
                                    min: 1,
                                    max: $scope.policies[i].max_retry_count
                                });
                                $('#retry_count').parsley('addConstraint', {
                                    maxcheck: $scope.policies[i].max_retry_count
                                });

                                $('#form').parsley('addItem', '#retry_interval');
                                $('#retry_interval').parsley('addConstraint', {
                                    required: true,
                                    type: 'number',
                                    min: $scope.policies[i].min_interval
                                });
                            }
                        }
                    }
            ...
            $scope.submit = function() {
                ...
                if ($('#form').parsley().isValid()) {
                    ...
                    consumerService.createOutput($scope.urlbase, $scope.apiversion, $scope.account_id, $scope.application_id, $scope.consumer_id, postdata).success(function(data) {
                        $scope.showMessage({
                            title: 'Success',
                            text: 'The output has been created!'
                        });
    ...
    .controller('DestinationEditCtrl', ['$scope', '$location', '$http', 'consumerService',
        function($scope, $location, $http, consumerService) {
            ...
            $scope.submit = function() {
                ...
                if ($('#form').parsley().isValid()) {
                    ...
                    consumerService.updateOutput($scope.urlbase, $scope.apiversion, $scope.account_id, $scope.application_id, $scope.consumer_id, $scope.output_id, postdata).success(function(data) {
                        $scope.showMessage({
                            title: 'Success',
                            text: 'The output has been updated successfully!'
                        });
                        $scope.changePage('destinations');
                    }).error(function(data) {
                        $scope.message = data.message || "Request failed";
            ...
            $scope.delete = function($output_id) {
                consumerService.deleteOutput($scope.urlbase, $scope.apiversion, $scope.account_id, $scope.application_id, $scope.consumer_id, $output_id).success(function(data) {
                    $scope.showMessage({
                        title: 'Success',
                        text: 'The destination has been deleted!'
                    });
                    $scope.changePage('destinations');
                }).error(function(data) {
                    $scope.message = data.message || "Request failed";
